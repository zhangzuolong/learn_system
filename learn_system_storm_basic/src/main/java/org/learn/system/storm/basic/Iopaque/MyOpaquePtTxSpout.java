...
public class MyOpaquePtTxSpout implements IOpaquePartitionedTransactionalSpout<MyMata>{
	...
	public class MyEmitter implements IOpaquePartitionedTransactionalSpout.Emitter<MyMata>{
		...
		public MyMata emitPartitionBatch(TransactionAttempt attempt,
			...
			System.out.println("MyEmitter emitPartitionBatch 启动一个新的PartitionBatch："+mata.toString());
			//发射
//			emitPartitionBatch(attempt,collector,partition,mata);
		...
		public int numPartitions() {
			System.out.println("MyEmitter numPartitions 得到分区数为-->"+5);
			return 5;
		}
